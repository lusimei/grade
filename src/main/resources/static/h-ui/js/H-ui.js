...
jQuery.postAjax = function (url, data, successfn) {
...
/* jQuery Ajax封装通用类 */
jQuery.fromAjax = function (url, data, successfn) {
    data = (data == null || data == "" || typeof (data) == "undefined") ? {
        "date": new Date().getTime()
    } : data;
    $.ajax({
...
!function ($) {
    $.Huimodalalert = function (info, speed) {
        ...
        if (speed == 0 || typeof(speed) == "undefined") {
            $(document.body).append('<div id="modal-alert" class="modal modal-alert radius">' + '<div class="modal-alert-info">' + info + '</div>' + '<div class="modal-footer"> <button class="btn btn-primary radius" onClick="$.Huimodal_alert.hide()">確定</button></div>' + '</div>');
            $("#modal-alert").fadeIn();
        } else {
